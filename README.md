...
| `MERCADOPAGO_WEBHOOK_SECRET` | Em produção | Valida a assinatura `x-signature` do webhook. **Obrigatória em produção** (falha no boot): sem ela o webhook responderia 500 a cada entrega — pagamentos capturados que nunca viram ingresso |
| `CRON_SECRET` | Não | Min. 16 chars; protege `GET /api/cron/*` (sem ela as rotas respondem 503) |
| `ALERT_WEBHOOK_URL` | Não | URL de webhook (Slack/Discord/genérico) que recebe alertas CRÍTICos de dinheiro (`alertCritical`): pagamento capturado sem ingresso, cobrança duplicada, preapproval duplicado, rate-limit degradado. Sem ela, os alertas ficam só em log de nível `error` |
